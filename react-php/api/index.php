...
function signup() {
            ...
            $email_check>0 && $username_check>0 && $password_check>0)
        {
            ...
            if($rowCount==0)
            {
                ...
                $db->query("INSERT INTO users(username,password,email,firstName,lastName,birthDate,type)
                            VALUES('$username','$password','$email','$Fname','$Lname','$DOB','$Type')");
...
{ 
       ...
       $rowCount=$result->num_rows;
        ...
        {
            $validMovies = [];
            $movies = $result->fetch_all();

            // foreach($movies as $movie){
                // if($movie->datetime )
            // $user_id=$movies->username;
